...
**`/junior/` — отдельная страница с 2026-08-25** (Ник: «просто что это даёт детям»), больше не редирект. Только про пользу для ребёнка (вода как норма, перенос спокойствия в жизнь, протокол внимания), без ECA/Headstart-питча и без заявлений о конкретной программе сертификации, пока кроссовер в Junior instructor не закрыт (см. [[Projects/nofins/roadmap]]). Ссылка на неё — из блока про детей на `/why/` («More for young divers»); в navbar не добавлять.
