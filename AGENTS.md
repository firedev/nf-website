...
**`/junior/` — отдельная страница с 2026-08-25** (Ник: «просто что это даёт детям»), больше не редирект. Только про пользу для ребёнка: dive reflex → спокойствие в жизни; дыхание/сердце/тело; поверхность + нырок + безопасное присутствие; 1:1 под характер и уровень; видимый прогресс без гонки; родители в курсе; бережное отношение к воде. Без ECA/Headstart-питча и без заявлений о Junior-сертификации / «методике Молчановой для детей», пока кроссовер в Junior instructor не закрыт (см. [[Projects/nofins/roadmap]]; RU-сырьё — [[Projects/nofins/junior-copy-source]]). Ссылка на неё — из блока про детей на `/why/` («More for young divers»); в navbar не добавлять.
