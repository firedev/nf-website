...
**`/junior/` — отдельная страница с 2026-08-25** (Ник: «просто что это даёт детям»), больше не редирект. Только про пользу для ребёнка. Четыре блока с kids.molchanova.school: осанка/лёгкие; фокус; плавать и нырять безопасно; любовь к воде. Плюс dive reflex → спокойствие в жизни; 1:1; видимый прогресс; родители в курсе. Референс-сырьё — [kids.molchanova.school](https://kids.molchanova.school/) → [[Projects/nofins/junior-copy-source]] (чужие цифры/Москва/методику Молчановой и Junior-сертификат на сайт не тащить до кроссовера). Без ECA/Headstart-питча. Ссылка на неё — из блока про детей на `/why/` («More for young divers»); в navbar не добавлять.
